...
export default function ClientComponent({
  ...
  // Optional: re-check vote status or refresh assets/hierarchy on mount or after actions
  useEffect(() => {
    // You could re-check hasVoted logic here if you later implement proper lockout
    setHasVoted(false); // placeholder — replace with real check if needed
  }, []);
  ...
  // Refresh assets after adding (simple version – could be optimized later)
  const handleAssetSuccess = () => {
    setShowAddAsset(false);
    window.location.reload(); // or fetch assets again
  };

  const handleParentSuccess = () => {
    setShowAddParentForm(false);
    window.location.reload(); // or refetch hierarchy
  };

  function getIndividualPercent(up: number | null, down: number | null): string {
          ...
          <div className="w-96 min-w-[360px] bg-gray-800/60 rounded-2xl p-7 border border-gray-700 text-center self-start mt-8">
            ...
            <div className="text-gray-300 text-base space-y-4 text-center">
              <p>
                ...
                <span className="text-red-400">es</span> are a combined total from the displayed
                company and all subsidiaries. <span className="text-yellow-400">Ranks</span> are a
                percentage between vote totals. The <span className="text-blue-400">(blue)</span>{' '}
                numbers are from the displayed company alone.
              </p>
              <p>
                Assets is a list of known assets owned by the displayed company. A lot of commonly
                used things are not standalone companies but rather, assets owned by the company.
              </p>
              <p>Mouse over tooltips show regional data for your region if available.</p>
            </div>
          </div>

          {/* Center: Main rank + vote */}
          <div className="w-full max-w-3xl text-center">
            <div className="mb-12">
              <div className="flex justify-center items-baseline gap-6 md:gap-5 mb-6">
                ...
                <span className="text-2xl md:text-2xl text-blue-400">({ownPercent}%)</span>
              </div>
              <p className="text-lg md:text-xl text-gray-400 mb-10">Rank Score - All (self)</p>
              ...
              <div className="flex justify-center items-baseline gap-12 md:gap-10 text-5xl md:text-6xl font-bold">
                <span className="text-green-600 group relative">
                  ...
                  <span className="text-blue-400 text-3xl md:text-4xl ml-4">({ownVotes.up})</span>
                </span>
                ...
                <span className="text-red-600 group relative">
                  ...
                  <span className="text-blue-400 text-3xl md:text-4xl ml-4">({ownVotes.down})</span>
                </span>
              </div>
            </div>
              ...
              <div className="flex justify-center items-center gap-12 md:gap-5">
                <button
                  ...
                  className={`group relative bg-green-700 text-white px-10 md:px-14 py-6 md:py-8 rounded-2xl text-5xl md:text-6xl font-bold transition shadow-xl ${
                    hasVoted ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-600'
                  }`}
                >
                  ↑
                ...
                <span className="text-3xl md:text-4xl font-bold text-gray-500">Vote</span>
                ...
                <button
                  ...
                  className={`group relative bg-red-700 text-white px-10 md:px-14 py-6 md:py-8 rounded-2xl text-5xl md:text-6xl font-bold transition shadow-xl ${
                    hasVoted ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-600'
                  }`}
                >
                  ↓
                  <span className="absolute left-1/2 -bottom-16 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity text-sm md:text-base text-gray-200 bg-black/90 p-3 md:p-4 rounded-xl whitespace-nowrap z-10">
            ...
            {assets.length === 0 ? (
              ...
              <ul className="space-y-4">
                {assets.map((asset, index) => (
                  ...
                  >
                    <div className="font-semibold text-xl text-gray-100">{asset.asset_name}</div>
                  </li>
                ))}
              </ul>
            )}
        ...
        <div className="lg:hidden space-y-6 px-3 sm:px-6 pb-6 sm:pb-8">
          ...
          <div className="text-center bg-gray-900/40 rounded-2xl p-6 border border-gray-700 mt-6">
            <div className="flex justify-center items-baseline gap-4 mb-4">
              ...
              <span className="text-2xl sm:text-3xl text-blue-400">({ownPercent}%)</span>
            </div>
            <p className="text-lg sm:text-xl text-gray-400 mb-6">Rank Score - All (self)</p>
            ...
            <div className="flex justify-center gap-12 sm:gap-16 text-5xl sm:text-6xl font-bold">
              <span className="text-green-500 group relative">
                ...
                <div className="text-blue-400 text-2xl sm:text-3xl mt-2">({ownVotes.up})</div>
              </span>
              ...
              <span className="text-red-500 group relative">
                ...
                <div className="text-blue-400 text-2xl sm:text-3xl mt-2">({ownVotes.down})</div>
              </span>
            </div>
          </div>
          ...
          <div className="text-center">
            <h3 className="text-3xl sm:text-4xl font-bold text-gray-300 mb-8">Have your say!</h3>
            ...
            {toastMessage && (
              <p className="mt-4 text-center text-yellow-300 text-lg">{toastMessage}</p>
            )}
          </div>
        </div>

        {/* Mobile side panels triggers */}
        ...
        >
          <div className="p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-bold text-white">How Ranks Are Formed</h3>
              <button
        ...
        >
          <div className="h-full overflow-y-auto overscroll-contain">
            <div className="p-6 pb-12">
              ...
              {assets.length === 0 ? (
                <p className="text-gray-400 py-10 italic text-center">No assets listed yet</p>
              ) : (
        ...
        <div className="mt-8 lg:mt-16">
          ...
          <div>
            ...
            {hierarchy.filter((h) => h.parent_id === companyId).length === 0 ? (
              <p className="text-center text-xl text-gray-500">No subsidiaries found</p>
            ) : (
              <ul className="space-y-6 max-w-2xl mx-auto">
                {hierarchy
                  ...
                  .map((h) => (
                    ...
                    >
                      ...
                      <span className="text-yellow-400 text-xl">
                        {getIndividualPercent(h.child?.vote_up ?? null, h.child?.vote_down ?? null)}
                      </span>
                    </li>
                  ))}
              </ul>
